...
export function TickersHoldingsTable({ holdings }: ITickersHoldingsTableProps) {
  ...
  const sortedTypes = Object.keys(groupedHoldings).sort((a, b) => {
    const totalA = groupedHoldings[a as ETickerType].reduce(
      (sum, h) => sum + h.totalValue,
      0,
    );
    const totalB = groupedHoldings[b as ETickerType].reduce(
      (sum, h) => sum + h.totalValue,
      0,
    );
  ...
  return (
    <div className="space-y-4">
      {sortedTypes.map((type) => {
        ...
        const totalValue = typeHoldings.reduce(
          (sum, h) => sum + h.totalValue,
          0,
        );
        const totalInvested = typeHoldings.reduce(
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  >
                    {typeHoldings.length}{" "}
                    {typeHoldings.length === 1 ? "ativo" : "ativos"}
                  </Badge>
                </div>
                <div className="text-right">
                  ...
                  <p
                    className={`text-xs ${totalProfitLoss >= 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                  >
                    ...
                    (
                    {formatPercentage(totalProfitLossPercent / 100, {
                      isPrivate: isPrivacyMode,
                    })}
                    )
                  </p>
                      ...
                      {typeHoldings.map((holding) => (
                        <TableRow key={holding.symbol}>
                          ...
                          <TableCell className="text-right">
                            {isPrivacyMode ? "•••" : holding.quantity}
                          </TableCell>
                          <TableCell className="text-right">
                    ...
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          ...
                          <p
                            className={`text-xs ${holding.profitLoss >= 0 ? "text-green-600 dark:text-green-600" : "text-red-600 dark:text-red-400"}`}
                          >
                            {!isPrivacyMode &&
                              (holding.profitLoss >= 0 ? "+" : "")}
                            {formatCurrency(holding.profitLoss, {
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        <div>
                          ...
                          <p
                            className={`font-medium ${holding.profitLoss >= 0 ? "text-green-600 dark:text-green-600" : "text-red-600 dark:text-red-400"}`}
                          >
                            {formatPercentage(holding.profitLossPercent / 100, {
                              isPrivate: isPrivacyMode,
                            })}
                          </p>
                        </div>
